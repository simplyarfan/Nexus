...
export default function Dashboard() {
  ...
  const fetchDashboardStats = async () => {
    try {
      const data = await get('/api/analytics/dashboard');
      setStats(data);
    } catch (error) {
  ...
  const statsData = [
    ...
    },
    {
      ...
      value: stats?.totalCandidates || '847',
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path
            strokeLinecap="round"
    ...
    },
    {
      ...
      value: stats?.scheduledInterviews || '23',
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path
            strokeLinecap="round"
  ...
  return (
    <>
      <Head>
        <title>Dashboard - Nexus AI Hub</title>
      </Head>
                        ...
                        >
                          ...
                          <div className="max-h-96 overflow-y-auto">
                            {notifications.map((notification, index) => (
                              <div
                                key={index}
                                className="p-4 hover:bg-accent transition-colors border-b border-border last:border-0"
                              >
